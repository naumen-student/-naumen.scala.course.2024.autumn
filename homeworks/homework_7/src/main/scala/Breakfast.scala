...
object Breakfast extends ZIOAppDefault {
  ...
  def makeBreakfast(eggsFiringTime: Duration,
    ...
    boiledWater <- ZIO.sleep(waterBoilingTime).map(_ => LocalDateTime.now).fork
    friedEggs <- ZIO.sleep(eggsFiringTime).map(_ => LocalDateTime.now).fork
    salad <- ZIO
      .sleep(saladInfoTime.tomatoTime
        .plus(saladInfoTime.cucumberTime))
      .map(_ => LocalDateTime.now).fork
    boiledWaterTime <- boiledWater.join
    tea <- ZIO.sleep(teaBrewingTime).map(_ => LocalDateTime.now).fork
    eggsTime <- friedEggs.join
    saladTime <- salad.join
    teaTime <- tea.join
  } yield Map(
    "eggs" -> eggsTime,
    "water" -> boiledWaterTime,
