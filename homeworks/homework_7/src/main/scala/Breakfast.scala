...
object Breakfast extends ZIOAppDefault {
  ...
  def makeBreakfast(eggsFiringTime: Duration,
    ...
    val saladTime = durationLong(saladInfoTime.cucumberTime.getSeconds + saladInfoTime.tomatoTime.getSeconds)
    for {
  ...
  private def addDurations(currentTime: LocalDateTime, durations: Duration*): LocalDateTime = {
    durations.foldLeft(currentTime)((date, duration) => date.plusSeconds(duration.getSeconds))
  }
