...
object ResuourceTraining extends ZIOAppDefault {
  ...
  def readData(filePath: String): IO[Throwable, String] =
    ZIO
      .acquireReleaseWith(ZIO
        .attempt(new BufferedReader(new FileReader(filePath))))(file =>
      ZIO
        .attempt(file.close())
        .orDie)(file => ZIO
      .attempt(file.readLine()))

  def writeData(filePath: String, data: String): ZIO[Any, Nothing, Unit] =
    ZIO
      .acquireReleaseWith(ZIO
        .succeed(
      new BufferedWriter(new FileWriter(filePath, false))))(file =>
      ZIO
        .succeed(file.close())
        .ignore)(file => ZIO
        .attempt(file.write(data))
        .map(_ => file.flush()).ignore)
