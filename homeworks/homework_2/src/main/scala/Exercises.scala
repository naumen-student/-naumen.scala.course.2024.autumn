object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def primeFactors(n: Int, div: Int, factors: Seq[Int]): Seq[Int] = {
            if (n <= 1) factors
            else if (n % div == 0) primeFactors(n / div, div, factors :+ div)
            else primeFactors(n, div + 1, factors)
        }

        primeFactors(number, 2, Seq()).distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy { case (_, (radius, density)) =>
            ((4 / 3) * radius * radius * radius * java.lang.Math.PI * density)
        }.map { case (name, _) =>
            name
        }
    }
