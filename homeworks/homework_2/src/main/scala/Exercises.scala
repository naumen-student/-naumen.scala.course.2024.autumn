object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        var k : Int = 2
        var resultSeq : Seq[Int] = Seq.empty[Int]
        var number = num
        while (k * k <= number) {
            if (number % k == 0) {
                resultSeq = resultSeq :+ k
            }
            while (number % k == 0) {
                number /= k
            }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortWith((x,y)=>4/3.0*Math.PI*x._2._1*x._2._1*x._2._1*x._2._2<4/3.0*Math.PI*y._2._1*y._2._1*y._2._1*y._2._2).map(_._1)
