object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = Range.inclusive(iFrom, iTo).filter(number => number % 3 == 0 || number % 5 == 0).sum
    ...
    def primeFactor(number: Int): Seq[Int] =
        (2 to (number / 2) + 1).foldLeft(Set.empty[Int]) {
            case (set, i) if number % i == 0 && !set.exists(i % _ == 0) => set + i
            case (set, _) => set
        }.toSeq
    ...
    type TransformVectorsFunction = (Vector2D, Vector2D) => Double
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, transformVectorsFunction: TransformVectorsFunction, rightVec0: Vector2D, rightVec1: Vector2D): Double =
        transformVectorsFunction(leftVec0, leftVec1) + transformVectorsFunction(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = 
        ballsArray.map {
            case(name, (radius, density)) => (name, java.lang.Math.pow(radius, 3) * java.lang.Math.PI * density * 4 / 3)
        }.toList.sortBy{
