object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        (2 to number).foldLeft(Seq.empty[Int])(
            (sequnce, index) =>
                if (number % index == 0 && !sequnce.exists(a => index % a == 0))
                    sequnce:+ index
                else sequnce
        )
    }
    ...
    private def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, function: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) = {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray
          .map(A => A match {
              case (name, (rad, pl)) => name -> 4 / 3 * Math.PI * Math.pow(rad, 3) * pl
          })
          .toSeq
          .sortBy {
