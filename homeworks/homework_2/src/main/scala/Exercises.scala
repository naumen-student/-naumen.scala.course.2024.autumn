...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def getFactors(num: Int, divisor: Int, factors: Seq[Int]): Seq[Int] = {
            if (num <= 1) factors
            else if (num % divisor == 0) {
                getFactors(num / divisor, divisor, if (!factors.contains(divisor)) factors :+ divisor else factors)
            }
            else getFactors(num, divisor + 1, factors)
        }

        getFactors(number, 2, Seq())
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq
          .sortBy { case (_, (radius, density)) => (4 / 3) * Math.PI * radius * radius * radius * density }
