...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def primeFactorRec(num: Int, divisor: Int, factors: Seq[Int]): Seq[Int] = {
            if (num <= 1) factors
            else if (num % divisor == 0) {
                if (!factors.contains(divisor)) {
                    primeFactorRec(num / divisor, divisor, factors :+ divisor)
                } else {
                    primeFactorRec(num / divisor, divisor, factors)
                }
            }
            else primeFactorRec(num, divisor + 1, factors)
        }

        primeFactorRec(number, 2, Seq())
    }
