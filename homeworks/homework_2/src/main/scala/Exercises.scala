...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        for {i <- 2 to number.toInt
             if number % i == 0 && (2 to Math.sqrt(i).toInt).forall(i % _ != 0)
             } yield i
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        var ballsWeight: Map[String, Double] = Map.empty[String, Double];
        for((name, (r, d)) <- ballsArray)
        {
            ballsWeight += (name -> d * 4.0 / 3 * java.lang.Math.PI * r * r * r);
        }
        ballsWeight = ListMap(ballsWeight.toSeq.sortBy(_._2):_*);
        ballsWeight.keys.toSeq;
    }
