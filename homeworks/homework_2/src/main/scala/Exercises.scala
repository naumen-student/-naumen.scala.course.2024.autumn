object Exercises {
    ...
    def cosBetween(vec0: Vector2D, vec1: Vector2D): Double = {
        val a = scalar(vec0, vec1) / abs(vec0) / abs(vec1)
        println(a)
        a
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val sorted = ballsArray.toSeq.sortBy(x => getWeight(x._2._1, x._2._2))
        val a = sorted.map(x => x._1)
        println(a)
        a
    }

    def getWeight(radius: Int, density: Double): Double = {
        4 * Math.PI * radius * radius * density
    }
