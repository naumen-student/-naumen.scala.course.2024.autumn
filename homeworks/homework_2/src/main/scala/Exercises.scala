...
object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    // Реализация функции для задания I
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    // Реализация функции для задания II
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def factors(n: Int, div: Int, seq: Seq[Int]): Seq[Int] = {
            if (n < 2) seq
            else if (n % div == 0) factors(n / div, div, seq :+ div)
            else factors(n, div + 1, seq)
        }
        factors(number, 2, Seq()).distinct
    }
