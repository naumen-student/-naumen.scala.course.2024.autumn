...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def primeFactorRec(n: Int, divisor: Int, primeFactors: Seq[Int]): Seq[Int] = {
            if (n == 1 || n == -1|| n == 0) primeFactors
            else if (n % divisor == 0) primeFactorRec(n / divisor, divisor, if (!primeFactors.contains(divisor)) primeFactors :+ divisor else primeFactors)
            else primeFactorRec(n, divisor + 1, primeFactors)
        }

        primeFactorRec(number, 2, Seq.empty[Int])
    }
