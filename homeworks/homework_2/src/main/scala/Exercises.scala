object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def getFactors(n: Int, divisor: Int = 2, factors: Seq[Int] = Seq()): Seq[Int] = {
            if (n < 2) factors
            else if (n % divisor == 0) getFactors(n / divisor, divisor, factors :+ divisor)
    ...
    def sumByFunc(
        ...
        operation: (Vector2D, Vector2D) => Double,
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def volume(radius: Int): Double = (4 / 3) * Math.PI * Math.pow(radius, 3)
        
        def mass(radius: Int, density: Double): Double = volume(radius) * density
        
        ballsArray.toSeq
            .sortBy { case (_, (radius, density)) => mass(radius, density) }
