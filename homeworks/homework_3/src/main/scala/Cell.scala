...
final class ReferenceCell(ix: Int, iy: Int, table: Table) extends Cell {
  @tailrec
  private def recursion(used: Set[Cell] = Set.empty[Cell]):String = {
    table.getCell(ix, iy) match {
      case Some(cell: ReferenceCell) => if (used.contains(cell)) "cyclic" else cell.recursion(used ++(Set(cell)))
      case Some(cell: Cell) => cell.toString
      case _ => "outOfRange"
    }
  }

  override def toString: String = recursion()
}