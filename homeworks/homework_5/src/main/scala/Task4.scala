...
object Task4 extends App {
  ...
  object EIO {
    ...
    def possibleError[A](f: => A): EIO[Throwable, A] = Try(f) match {
      ...
      case Failure(b) => EIO(Left(b))
    }

    implicit def monad[E]: MonadError[EIO, E] = new MonadError[EIO, E] {

      override def pure[A](value: A): EIO[E, A] = new EIO(Right(value))

      override def flatMap[A, B](fa: EIO[E, A])(f: A => EIO[E, B]): EIO[E, B] = fa.value match {
        case Left(a) => EIO(Left(a))
        case Right(b) => f(b)
      }

      override def raiseError[A](fa: EIO[E, A])(error: => E): EIO[E, A] = new EIO(Left(error))

      override def handleError[A](fa: EIO[E, A])(handle: E => A): EIO[E, A] = fa.value match {
        case Left(a) => EIO(Right(handle(a)))
        case Right(b) => EIO(Right(b))
      }
    }
  }
