...
object Task2 extends App {
  ...
  case class DegreeAngle(a: Double) {
    val angel: Double = {
      val normalized = a % 360
  ...
  object SquareMatrix {
    implicit def monoid[A: Monoid]: Monoid[SquareMatrix[A]] = new Monoid[SquareMatrix[A]] {
      override def empty: SquareMatrix[A] = {
        val emptyElem = implicitly[Monoid[A]].empty
        val emptyTuple = (emptyElem, emptyElem, emptyElem)
        SquareMatrix[A]((emptyTuple, emptyTuple, emptyTuple))
      }
