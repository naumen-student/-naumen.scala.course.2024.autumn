...
object Task2 extends App {
  ...
  object RadiusVector {
    implicit val monoid: Monoid[RadiusVector] = new Monoid[RadiusVector] {
      ...
      override def combine(a: RadiusVector, b: RadiusVector): RadiusVector = new RadiusVector(a.x + b.x, a.y + b.y)
    }
  }
  ...
  object DegreeAngle {
    implicit val monoid: Monoid[DegreeAngle] = new Monoid[DegreeAngle] {
      ...
      override def combine(x: DegreeAngle, y: DegreeAngle): DegreeAngle
                                                            = new DegreeAngle(x.angel + y.angel % 360 + 360 % 360)
      //                                              ангелов складываем, лол     ^        ^
      // а еще на это задание тесты нестабильные, в них постояно угол оказывается отрицательным, что противоречит условию.
    }
  }
  ...
  object SquareMatrix {
    implicit def monoid[A: Monoid]: Monoid[SquareMatrix[A]] = new Monoid[SquareMatrix[A]] {
      override def empty: SquareMatrix[A] = new SquareMatrix[A](
        (Monoid[A].empty, Monoid[A].empty, Monoid[A].empty),
        (Monoid[A].empty, Monoid[A].empty, Monoid[A].empty),
        (Monoid[A].empty, Monoid[A].empty, Monoid[A].empty)
      )

      override def combine(x: SquareMatrix[A], y: SquareMatrix[A]): SquareMatrix[A] = new SquareMatrix[A](
