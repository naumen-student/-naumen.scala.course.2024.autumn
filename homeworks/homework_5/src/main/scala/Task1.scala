...
object Task1 extends App {
	...
	// Типовой класс Show для отображения
	trait Show[-A] {
		def show(a: A): String
	}
	...
	object ShowInstance {
		...
		implicit def boxShow[A: Show](implicit showInstance: Show[A]): Show[Box[A]] = new Show[Box[A]] {
			def show(box: Box[A]): String = box match {
				case BoxWith(value) => s"${showInstance.show(value)} в коробке"
